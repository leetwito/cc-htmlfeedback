...
<div id="fb-popover">
  ...
  let pending = null, uid = 0, side = 'right', history = [], hpos = -1;
  ...
  function place(rect){
    ...
    let left = side === 'left' ? (window.scrollX + rect.right - w) : (window.scrollX + rect.left);
  ...
  function hidePop(){ closePop(); clearPending(); }
  ...
  function cardHTML(f){
    ...
    const st = statusOf(f);
    const pill = st ? '<span class="fb-status ' + stClass(st) + '">' + st.replace('-', ' ') + '</span>' : '';
  ...
  function applyStatus(card, f){
    ...
    if(pill){ pill.className = 'fb-status ' + stClass(st); pill.textContent = st.replace('-', ' '); }
  ...
  function reanchor(f){
    ...
    let n;
    while((n = w.nextNode())){
      if(panel.contains(n) || pop.contains(n) || launch.contains(n)) continue;
